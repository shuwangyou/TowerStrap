...
/**
 * 计算两点之间的距离、向量和角度（正上为零度）
 * @param {cc.Vec2} startPos 起点
 * @param {cc.Vec2} endPos 终点
 * @return {Object} {
        distance: Number,
        vector: cc.Vec2,
        degree: Number,
 ...
 */
function analyzePosOffset(startPos, endPos) {
    let dx = endPos.x - startPos.x,
        dy = endPos.y - startPos.y,
        dir = cc.v2(dx, dy),
        //根据朝向计算出夹角弧度
        angle = dir.signAngle(cc.v2(0, 1)),
        //将弧度转换为欧拉角
        degree = angle / Math.PI * 180

    return {
        distance: startPos.sub(endPos).mag(),
        vector: cc.v2(dx, dy),
        degree: degree,
    }
}
